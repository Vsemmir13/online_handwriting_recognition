...
class PointSet:
    ...
    def get_all_points(self):
        ...
        x, y = [], []
        for g in groups:
            x.extend([p.x for p in g])
            y.extend([p.y for p in g])
        return x, y
        ...
        for s in strokes:
            for i in range(len(s)):
                if (
                    i == 0
                    or i == len(s) - 1
                    or Line(s[i], s[i - 1 - removed]).length() > d_th
                ):
                    removed = 0
                    ret.append(s[i])
                else:
        ...
        for s in strokes:
            for i in range(len(s)):
                if i == 0 or i == len(s) - 1:
                    ...
                    cs = Line(s[i - 1 - removed], s[i]).cosine_similarity(
                        Line(s[i], s[i + 1])
                    )
                    if cs < cos_th:
                        removed = 0
    ...
    def generate_features(self, preprocess=None, pad=0, add_pad=0, fset=1):
        ...
        for l in lines:
            if fset == 1:
                ...
                features.append(l.get_features_norm())
    ...
    def __repr__(self):
        return "<PointSet w={} h={} points_len={}>".format(
            self.w, self.h, len(self.points)
        )
...
class Point:
    ...
    def __repr__(self):
        return "<Point stroke={} time={} x={} y={}>".format(
            self.stroke, self.time, self.x, self.y
        )
